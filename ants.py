...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if not self.ant:
                self.ant = insect
            elif self.ant.is_container is True and self.ant.can_contain(insect):
                self.ant.contain_ant(insect)
            elif insect.is_container is True and insect.can_contain(self.ant):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if not isinstance(insect, QueenAnt) or (isinstance(insect, QueenAnt) and insect.queen_id != 1):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        target_bee = self.place
        distance = 0
        while target_bee is not hive:
            if target_bee.bees and distance == 0:
                return random_or_none(target_bee.bees)
            if distance <= self.max_range and distance > self.min_range and target_bee.bees:
                return random_or_none(target_bee.bees)
            target_bee = target_bee.entrance
            distance += 1
        return None
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        if not self.place.bees:
            return None
        if self.digesting != 0 and self.time_to_digest == 3:
            self.digesting -= 1
            return self.action
