...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return complete_apply(operator, operand, env)
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        formal_parameter = self.formals 
        return self.env.make_child_frame(formal_parameter, args)
        # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            elif not clause.first and not expressions.second:
                return None 
            elif clause.first is not False:
                if clause.second:
                    return eval_all(clause.second, env)
                else:
                    return test 
            else:
                do_cond_form(expressions.second, env)
            # END PROBLEM 14
        expressions = expressions.second
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        env.define(target.first, MacroProcedure(target.second, body, env))
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM 5
                operator = original_scheme_eval(first, env) 
                check_procedure(operator)
                if not isinstance(operator, MacroProcedure):
                    ...
                    result = scheme_eval(operator.apply_macro(rest, env), env)
